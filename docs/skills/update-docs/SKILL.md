---
name: update-docs
description: Analyze codebase and update AGENTS.md to reflect current project state. Use when project structure, dependencies, or architecture changes.
argument-hint: ""
...
2. **Zjisti tech stack** — přečti `package.json`, `tsconfig*.json`, `vite.config.*`, `eslint.config.*` a další konfigurační soubory; zapiš přesné verze
...
- Používej cesty relativně ke kořenu repozitáře (`src/main.tsx`, ne `./src/main.tsx`)
- Piš anglicky (AGENTS.md je určen pro mezinárodní AI agenty)

## Aktualizace CLAUDE.md

Po vytvoření/aktualizaci `AGENTS.md` zkontroluj `CLAUDE.md` a zajisti, že obsahuje referenci:

```
@AGENTS.md
```

Tato `@` reference zajistí, že Claude Code automaticky načte obsah `AGENTS.md` jako kontext. Pokud reference chybí, přidej ji na začátek `CLAUDE.md` (před první sekci).
